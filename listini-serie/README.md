...
## Invio a IWG-Preventivatore (15.09.2026)
Sezione "Invio a IWG-Preventivatore" nel programma: converte tutte le tipologie della serie selezionata in prodotti del catalogo del preventivatore (tabella `catalogo_prodotti`, stesso formato di admin.html → "Importa listini JSON": id `LINEA_IWG_67ID_<cod>` / `LINEA_IWG_77ID_<cod>` / `LINEA_IWG_S140_<cod>`, gamma IWG67ID / IWG77ID / IWG S140, famiglia Portoncini IWG / Alzante Scorrevole IWG / Scorrevoli, dim1 = altezza, dim2 = larghezza, `grid["HxL"]` = prezzo di listino senza vetro). Due modi: invio diretto con login administrator (upsert per id) oppure download del JSON da importare in admin.html. Dalla pagina pubblicata su claude.ai le chiamate al database sono bloccate: usare il file HTML locale o il JSON.
