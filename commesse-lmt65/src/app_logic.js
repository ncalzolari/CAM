...
function lavFX(L){ const out=[]; posFX(L).forEach(x=>LAV_FX.forEach(s=>out.push(Object.assign({x,v3:'2'},s)))); return out; }
...
function lavCerniere(L, specchio){
  ...
  const out=[];
  a.forEach(x=>out.push(Object.assign({x,v3:'2'},LAV_CERN.d7)));
  [37.5,51.5,65.5,79.5].forEach(x=>out.push(Object.assign({x:x+(specchio?2:0),v3:'2'},LAV_CERN.d3)));
  b.forEach(x=>out.push(Object.assign({x:Math.round((L-x)*10)/10,v3:'2'},LAV_CERN.d7)));
  [37.5,51.5,65.5,79.5].forEach(x=>out.push(Object.assign({x:Math.round((L-x-(specchio?0:2))*10)/10,v3:'2'},LAV_CERN.d3)));
  return out;
}
...
const SC_PAIR16 = (x)=>[{x:x-8,w:'#0',v1:'1.5',v2:'',v3:'2',y:10,z:-20.5,f:'1',ut:'4',descr:'Scontro alza-anta D3'},
...
function componiMatriceBase(r, t, agg){
  ...
  [1,2].forEach(k=>{
    ...
    if(conDren&&k===1){ const pos=posizioniDrenaggio(L);
      lav=pos.map(x=>lavDrenaggio(t.serie,'telaio',x)); capDrenaggio(t.serie,accessori,pos.length); }
  ...
  for(let rr=0;rr<hs.length-1;rr++) for(let c=0;c<ws.length;c++){
    ...
    if(conDren){ const pos=posizioniDrenaggio(lunT);
      ...
      lavT=pos.map(x=>lavDrenaggio(t.serie,tipoT,x)); capDrenaggio(t.serie,accessori,pos.length); }
          ...
          if(inVista && conForo8 && ferrAR && montanteTelaio){          // cerniere angolari lato cerniere
            ...
            if(!dueAnte){                                              // scontri Maico (1 anta)
              ...
              if(attivo){
                ...
                [mm/2-19, mm/2+19].forEach(x=> lav.push({x:Math.round(x*10)/10, w:'#0',v1:'1.5',v2:'',v3:'2',
...
function svgPezzoLav(p){
  ...
  ['1','2','3','4'].forEach(f=>{
    ...
    const righe = lav.map(l=>{
      ...
      return `<div style="font-size:.72rem"><b class="num">${geo}</b> a X ${l.x} · Y ${l.y} · Z ${l.z||0} · ut.${l.ut} <span style="color:var(--acciaio)">${l.descr||''}</span></div>`;
    }).join('');
