...
function lavDef(chiave, serie, art){
  ...
  return {w:d.w||'#0', v1:String(d.v1), v2:(d.v2==null?'':String(d.v2)), v3:String(d.v3||2), y:parseFloat(d.y), z:parseFloat(d.z)||0, f:String(d.f), ut:String(d.ut),
          descr:(d.descr||chiave) + (ov ? ` [lib.${art}]` : (mod ? ` [lib.${serie}]` : ''))};
}
...
// ---------- ferramenta Maico delle celle apribili nel costruttore (patch 8) ----------
    ...
    if(v==='F'){
      ...
      if(v==='2A' || v==='PF2'){
        ...
        for(let k=0;k<3;k++) spingi(anta,'Montante battente',`${id}-ANTH`,ah,'45-45',
          (agg.conAnte? [lavDrenaggio(t.serie,'antaMont', k%2===0 ? 218 : Math.round((ah-218)*10)/10)] : []).concat(FER.ant[id]&&FER.ant[id].k===k ? FER.ant[id].lav : []));
        ...
        [0,1].forEach(k=>spingi(anta,'Montante battente',`${id}-ANTH`,ah,'45-45',
          (agg.conAnte? [lavDrenaggio(t.serie,'antaMont', k===0 ? 218 : Math.round((ah-218)*10)/10)] : []).concat(FER.ant[id]&&FER.ant[id].k===k ? FER.ant[id].lav : [])));
...
function iniziaSelettori(){
  ...
  ['#r-montante-s140','#r-soglia-s140','#r-lato-s140'].forEach(id=>$(id).addEventListener('change', risolviTipS140));
...
function leggiRigaCorrente(quiet){
  ...
  return {tid:t.id, codTip, mat, battuta, mano, hm, blocal, telaio:$('#r-telaio').value, anta:$('#r-anta').value,
    ...
    colore: ($('#r-colore') ? $('#r-colore').value.trim() : '') || ''};
}
  ...
  tb.innerHTML = righe.map((r,i)=>{
    if(r.tipo==='composta'){
      ...
      return `<tr><td class="n">${i+1}</td><td>${r.m.serie}</td>
        ...
        <td class="no-stampa"><button class="spoglio" onclick="schedaProdotto(${i})" title="Scheda prodotto">📄</button>
    ...
    return `<tr><td class="n">${i+1}</td><td>${t.serie}</td><td>${mini} <span class="num"><b>${r.codTip||t.cod}</b></span> ${t.nome}</td>
      ...
      <td class="no-stampa"><button class="spoglio" onclick="modificaRiga(${i})" title="Modifica">✎</button>
        <button class="spoglio" onclick="schedaProdotto(${i})" title="Scheda prodotto">📄</button>
        <button class="spoglio" onclick="rimuovi(${i})" title="Elimina">✕</button></td></tr>`;
  }).join('');
...
window.modificaRiga = function(i){
  ...
  if($('#r-colore')) $('#r-colore').value = r.colore||'';
  $('#r-q').value = r.q;
  if(r.mat){
...
// ---------- scheda prodotto (per riga: il prodotto come configurato, con i dati che alimentano la commessa) ----------
function calcolaRigaSingola(riga){
  // calcola distinta/accessori/guarnizioni/vetri SOLO per questa riga (q forzata a 1), riusando calcolaCommessa()
  // sullo stesso array `righe` del programma: lo svuota, ci mette una copia della riga, calcola, poi lo ripristina.
  const salvate = righe.slice();
  righe.length = 0;
  righe.push(Object.assign({}, riga, {q: 1}));
  const calc = calcolaCommessa();
  righe.length = 0;
  salvate.forEach(r => righe.push(r));
  return calc;
}
function nomeTipologiaScheda(r, t){
  // stessa risoluzione di nome/codice di calcolaCommessa() (battuta stulp/nodo stretto, base telaio a terra, mano, codice riga)
  let nome = t.nome, cod = r.codTip || t.cod;
  if(r.battuta==='stulp' && DATI.stulp_map[r.tid]) nome += ' — STULP';
  else if(r.battuta==='montante') nome += ' — NODO STRETTO';
  if(r.base==='telaio' && DATI.pb_telaio[r.tid]) nome += ' — TELAIO A TERRA';
  if(r.mano) nome += ' — ' + r.mano.toUpperCase();
  return {nome, cod};
}
window.schedaProdotto = function(i){
  const r = righe[i]; if(!r) return;
  let corpo;
  if(r.tipo==='composta'){
    const d = distintaComposta(r.m, {dren: $('#c-dren').checked});
    const artSet = [...new Set(d.pezzi.map(p=>p.art))];
    corpo = schedaHtml({
      titolo: `Composizione ${r.codTip}`, rif: `costruttore ${r.m.serie} — griglia ${r.m.cols}×${r.m.rows}`,
      L: r.L, H: r.H, vetro: r.vetro, colore: r.colore, q: r.q, cod: r.codTip,
      prospetto: svgProspettoMatrice(r.m.ws, r.m.hs, r.m.celle, null, r.m.giunte, r.m.serie, false, r.m.giunteO),
      profArt: artSet, accessori: d.accessori, guarnizioni: d.guarnizioni, vetri: d.vetri
    });
  } else {
    const t = DATI.tipologie.find(x=>x.id===r.tid); if(!t) return;
    const {nome, cod} = nomeTipologiaScheda(r, t);
    const calc = calcolaRigaSingola(r);
    const artSet = [...new Set(calc.pezzi.map(p=>p.art))];
    const prospForma = (DATI.pb_telaio[t.id] && r.base==='telaio') ? (t.forma==='P1'?'1':'2') : t.forma;
    corpo = schedaHtml({
      titolo: nome, rif: t.rif || '', L: r.L, H: r.H, vetro: r.vetro, colore: r.colore, q: r.q, cod,
      prospetto: r.mat ? svgProspettoMatrice(r.mat.ws, r.mat.hs, r.mat.celle, null, r.mat.giunte||r.mat.giunta, r.serie||t.serie, false, r.mat.giunteO)
                        : svgProspetto(prospForma, r.L, r.H, null, r.mano||'dx', r.serie||t.serie),
      profArt: artSet, accessori: calc.accessori, guarnizioni: calc.guarnizioni, vetri: calc.vetri,
      avviso: t.avviso
    });
  }
  $('#out-scheda').innerHTML = corpo;
  _sezSalvate = [];
  document.querySelectorAll('section').forEach(s=>{
    if(s.id==='sez-scheda') return;
    _sezSalvate.push([s, s.style.display]);
    s.style.display = 'none';
  });
  $('#sez-scheda').style.display = 'block';
  window.scrollTo(0,0);
};
function chiudiScheda(){
  const sz = document.querySelector('#sez-scheda'); if(sz) sz.style.display = 'none';
  (_sezSalvate||[]).forEach(([s,d])=>{ s.style.display = d; });
  _sezSalvate = null;
  window.scrollTo(0,0);
}
function schedaHtml(d){
  const sezioni = d.profArt.map(art => `<div class="scheda-card">${svgSezione(art)}</div>`).join('');
  const accRows = Object.entries(d.accessori||{}).map(([k,v])=>{ const [art,desc]=k.split('|');
    const q = v.pz ? v.pz+' pz' : '—'; const extra = v.mm ? ` (${fmt(v.mm)} mm)` : '';
    return `<tr><td class="num">${art}</td><td>${desc}${extra}</td><td class="n">${q}</td></tr>`; }).join('');
  const guaRows = Object.entries(d.guarnizioni||{}).map(([k,mm])=>{ const [art,desc]=k.split('|');
    return `<tr><td class="num">${art}</td><td>${desc}</td><td class="n">${(mm/1000).toFixed(1)} m</td></tr>`; }).join('');
  const vetRows = (d.vetri||[]).map(v=>`<tr><td class="n">${v.q}×</td><td>${v.tip||''} ${v.unita?'— '+v.unita:''}</td><td class="n">${fmt(v.l)} × ${fmt(v.h)} mm</td><td class="n">${v.sp||d.vetro} mm</td></tr>`).join('');
  return `
    <div class="scheda-testata">
      <div class="scheda-fatti">
        <div><b>${d.cod}</b><span>codice</span></div>
        <div><b>${d.L} × ${d.H}</b><span>L × H mm</span></div>
        <div><b>${d.q}</b><span>quantità</span></div>
        <div><b>${d.vetro||'—'} mm</b><span>vetro</span></div>
        <div><b>${d.colore||'da specificare'}</b><span>colore</span></div>
      </div>
      <div style="max-width:340px">${d.prospetto}</div>
    </div>
    <p><b>${escHtml(d.titolo)}</b>${d.rif?' — <span class="nota-piccola">'+escHtml(d.rif)+'</span>':''}</p>
    ${d.avviso ? `<div class="avviso">${escHtml(d.avviso)}</div>` : ''}
    <h3 style="font-size:.8rem;letter-spacing:.1em;color:var(--rosso);margin:1rem 0 .4rem">SEZIONI PROFILO</h3>
    <div class="scheda-griglia">${sezioni}</div>
    <h3 style="font-size:.8rem;letter-spacing:.1em;color:var(--rosso);margin:1rem 0 .4rem">ACCESSORI E FERRAMENTA</h3>
    <table><thead><tr><th>Articolo</th><th>Descrizione</th><th class="n">Q.tà</th></tr></thead><tbody>${accRows||'<tr><td colspan="3" class="nota-piccola">nessuno</td></tr>'}</tbody></table>
    <h3 style="font-size:.8rem;letter-spacing:.1em;color:var(--rosso);margin:1rem 0 .4rem">GUARNIZIONI</h3>
    <table><thead><tr><th>Articolo</th><th>Descrizione</th><th class="n">Sviluppo</th></tr></thead><tbody>${guaRows||'<tr><td colspan="3" class="nota-piccola">nessuna</td></tr>'}</tbody></table>
    <h3 style="font-size:.8rem;letter-spacing:.1em;color:var(--rosso);margin:1rem 0 .4rem">VETRO</h3>
    <table><thead><tr><th class="n">Q.tà</th><th>Posizione</th><th class="n">Misura</th><th class="n">Spessore</th></tr></thead><tbody>${vetRows||'<tr><td colspan="4" class="nota-piccola">nessuno</td></tr>'}</tbody></table>`;
}
...
const bcs=$('#btn-chiudi-scheda'); if(bcs) bcs.addEventListener('click', chiudiScheda);
const bss=$('#btn-stampa-scheda'); if(bss) bss.addEventListener('click', ()=>window.print());
