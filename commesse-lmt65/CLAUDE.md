...
- **Ferramenta celle composta** (patch 8, `ferramentaCelleComposta()`): ogni cella apribile è la tipologia singola equivalente su un "pezzo virtuale" (anta+43, ante a 21,5 dagli estremi) e le lavorazioni delle funzioni validate (`lavCerniere`, `lavScontri*`, `lavScontroForbice`, `lavMartellina`, regole stulp) vengono spostate sul pezzo reale (stipite passante: origine 0; montante T: inizio a `trvV_tel`; traverso T: inizio a `xl+trvO`). Sui T lo scontro d'angolo sta a `NODO_T` = 55,5 dal nodo; Y/faccia dei T sono eccezioni di libreria `@B23609C`/`@B23608C` (sc_angolo Y12, cerniere Y66,2 F4). Tutto marcato `[DA VERIFICARE composta]` finché non validato col diff sul job 220_26 (importa job XML). Vasistas esclusi (sospeso 3).
