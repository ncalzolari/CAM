...
(async () => {
  ...
  const out = await p.evaluate((casi)=>{
    ...
    const m = mostra();
    const pezzi = m.calc.pezzi.map(x=>({cod:x.cod, art:x.art, desc:x.desc, mm:x.mm, lav:x.lav.map(l=>`${l.descr}|x${l.x}|y${l.y}|z${l.z}|f${l.f}|ut${l.ut}`)}));
  ...
  const checks = [
    ...
    ['tutta la ferramenta di cella marcata', out.pezzi.every(x=>x.lav.every(l=>/Drenaggio|^FX|DA VERIFICARE composta/.test(l)))],
  ];
