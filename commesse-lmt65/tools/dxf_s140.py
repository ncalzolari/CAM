# Sezioni profili AluK S140: DXF ricevuti (formato piatto LINE/ARC, layer unico) -> data/dxf_sez_s140.json
# Stesso parser di dxf2svg.py (D67/D77): nessun filtro layer necessario (un solo layer '0' in tutti i file).
# Include sia i codici base (U10140, U10400...) sia le varianti con suffisso (U10140_A...H/_L, U10000_A...D, ecc.):
# queste ultime non sono ancora agganciate a un profilo di dati_s140.json (nessun codice profilo le nomina),
# restano disponibili in libreria per un uso futuro (varianti di lunghezza/lavorazione dello stesso profilo).
import math, glob, os, json
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SRC = os.path.join(ROOT, 'data', 'dxf', 'S140')
...
if __name__ == '__main__':
    res = {}
    for fn in sorted(glob.glob(os.path.join(SRC, '*.DXF')) + glob.glob(os.path.join(SRC, '*.dxf'))):
        k = os.path.basename(fn)[:-4]
        s = svg(fn)
        res[k] = s; print(f"{k:12} {s['w']:7} x {s['h']:6}  n={s['n']}")
    json.dump(res, open(os.path.join(ROOT, 'data', 'dxf_sez_s140.json'), 'w'))
    print(len(res), 'sezioni ->', 'data/dxf_sez_s140.json')
