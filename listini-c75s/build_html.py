...
<section><h2>Parametri (modificabili: tutte le griglie si ricalcolano)</h2><div class="corpo">
  ...
  <div class="azioni"><button id="btn-reset" class="spoglio" style="background:none;color:var(--rosso);border:none;padding:0">ripristina valori di consegna</button>
    <span class="nota">Le modifiche restano salvate in questo browser. Prezzi profili: taglio termico = codici B (art. <span id="art-tt"></span>), normali = N/K e fermavetri (art. <span id="art-n"></span>).</span></div>
</div></section>
...
function disegnaParam(){ $('#param').innerHTML = PARAM_DEF.map(([k,lab,st])=>`<div><label>${lab}</label><input class="giallo num" type="number" step="${st}" data-p="${k}" value="${P[k]}"></div>`).join('');
  document.querySelectorAll('[data-p]').forEach(i=>i.addEventListener('change', ()=>{ P[i.dataset.p]=parseFloat(String(i.value).replace(',','.'))||0; salva(); disegnaTipo(); })); }
function disegnaTipo(){
  ...
  document.querySelectorAll('[data-fq]').forEach(i=>i.addEventListener('change', ()=>{ fr[+i.dataset.fq][1]=parseFloat(i.value)||0; salva(); disegnaTipo(); }));
...
['#q-l','#q-h'].forEach(id=>$(id).addEventListener('change', disegnaTipo));
