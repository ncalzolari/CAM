...
<section><h2>Tipologia</h2><div class="corpo">
  ...
  <div class="riq">
    ...
    <div><h3 style="font-size:.8rem;letter-spacing:.1em;color:var(--rosso);margin-bottom:.4rem">FERRAMENTA MAICO — kit per la misura interrogata</h3><div class="nota" style="margin-bottom:.3rem">Kit dipendente da FFB/FFH (anta − 20) secondo il poolfile WinPlus del 14/09/2026. Prezzi = netto Maico 2025, già scontati: lo sconto accessori dei parametri vale solo per accessori e guarnizioni AluK, non per la ferramenta. Prezzo, codice e quantità delle voci fisse sono modificabili (celle gialle) e valgono per tutte le misure.</div><div class="scroll"><table id="tab-ferr"></table></div></div>
...
<section id="sez-iwg"><h2>Invio a IWG-Preventivatore</h2><div class="corpo">
  ...
  <p class="nota">Invia le 5 tipologie C75S come prodotti della gamma IWG75 (tabella catalogo_prodotti, formato "Importa listini JSON" di admin.html): griglia H×L dei prezzi di listino senza vetro, aggiornamento per id. Serve un utente con ruolo administrator. Dalla pagina pubblicata l'invio diretto è bloccato: scarica il JSON e importalo da admin → Importa listini JSON.</p>
...
const IWG_NOMI = {F1:'Finestra 1 anta AR (C75S)', F2:'Finestra 2 ante AR (C75S)', PF1:'Portafinestra 1 anta AR (C75S)', PF2:'Portafinestra 2 ante AR (C75S)', FISSO:'Fisso (C75S)'};
function prodottiIWG(){ return DATI.ordine.map((k,i)=>{ const t = DATI.tip[k], c = coef(k), Ls = range(t.L), Hs = range(t.H), grid = {};
    Hs.forEach(H=>Ls.forEach(L=>{ grid[H+'x'+L] = costo(c,L,H).listino; }));
    return {id:`LINEA_IWG_75_C75S_${k}`, materiale:'Alluminio', famiglia:'Battenti IWG', gamma:'LINEA IWG 75', gammaLabel:'IWG75', code:`C75S ${k}`, name:IWG_NOMI[k]||t.nome, unit:'cad.',
            dim1Label:'Altezza', dim2Label:'Larghezza', dim1Values:Hs, dim2Values:Ls, grid, ordine:50+i, attivo:true}; }); }
...
function workbookIWG(prod){ const X = XLSX, wb = X.utils.book_new();   // formato impostazioni.html → Listini → "Import serie (xlsx)"
  ...
  return wb; }
$('#btn-iwg-xlsx').addEventListener('click', ()=>{ if(senzaXlsx()) return; const prod = prodottiIWG(); scarica(xlsxBytes(workbookIWG(prod)), 'IWG_C75S.xlsx'); $('#iwg-log').textContent = `${prod.length} prodotti nel file IWG_C75S.xlsx: nel preventivatore apri Impostazioni → Listini → "Import serie (xlsx)".`; });
$('#btn-iwg-json').addEventListener('click', ()=>{ const prod = prodottiIWG(); scarica(JSON.stringify(prod, null, 1), 'IWG_listini_C75S.json'); $('#iwg-log').textContent = `${prod.length} prodotti nel file IWG_listini_C75S.json: importalo da admin.html → Importa listini JSON.`; });
$('#btn-iwg-invia').addEventListener('click', async ()=>{ const log = $('#iwg-log'); const prod = prodottiIWG();
  ...
  try{ const sb = window.supabase.createClient(IWG_SB.url, IWG_SB.key, {auth:{persistSession:false}});
    ...
    log.textContent = `Inviati ${prod.length} prodotti (IWG75) a IWG-Preventivatore: ${prod.map(p=>p.code).join(', ')}.`;
